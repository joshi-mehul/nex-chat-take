...
import type { AnnounceMessage, FlowEdge, FlowNode, SelectionState, Vector2, ViewportState } from "@types/flow";
...
export const useFlowStore = create<FlowState>((set, get) => ({
  ...
  removeNodes: (ids) => {
    set(produce<FlowState>((draft) => {
      ...
      draft.edges = draft.edges.filter(e => !removedNodeSet.has(e.fromNodeId) && !removedNodeSet.has(e.toNodeId));
  ...
  removeEdges: (ids) => {
    set(produce<FlowState>((draft) => {
      ...
      draft.selection.edgeIds = draft.selection.edgeIds.filter(id => !ids.includes(id));
    }));
  ...
  completeConnect: (toNodeId) => {
    ...
    set(produce<FlowState>((draft) => {
      draft.edges.push({ id, fromNodeId: connecting.fromNodeId, toNodeId, label: undefined });
  ...
  cancelConnect: () => {
    set(produce<FlowState>((draft) => { draft.connecting = null; }));
  },
  ...
  announce: (text, politeness = "polite") => {
    ...
    set(produce<FlowState>((draft) => { draft.announcer.push(msg); }));
    // Auto-trim queue
    setTimeout(() => {
      set(produce<FlowState>((draft) => {
