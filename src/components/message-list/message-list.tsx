...
import { User, Bot, AlertCircle, Copy, ThumbsUp, ThumbsDown } from 'lucide-react';
...
const MessageItem = memo(({ message }: { message: Message }) => {
  ...
  return (
    <div className={`flex gap-4 max-w-4xl mx-auto p-4 animate-fade-in ${
      ...
      <div className={`flex flex-col ${message.role === 'user' ? 'items-end' : 'items-start'} max-w-xs sm:max-w-md lg:max-w-2xl`}>
        <div className={`
...
const MessageList: React.FC<MessageListProps> = memo(({ messages, isLoading, error }) => {
      ...
      {isLoading && (
        <div className="flex gap-4 max-w-4xl mx-auto p-4 animate-fade-in">
          ...
          <div className="flex flex-col items-start">
            ...
            <span className="text-xs text-gray-500 dark:text-gray-400 mt-2 ml-4">AI is typing...</span>
          </div>
        </div>
      )}
