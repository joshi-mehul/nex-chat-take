...
import { computeEdgePath, drawCubic, drawEdgeLabel, nearestPortPoint } from "@utils/canvasUtils";
...
export const drawEdges = (
  ...
  edges.forEach(edge => {
    ...
    const fromPoint = nearestPortPoint(from, to.position);
    ...
    const { cx1, cy1, cx2, cy2 } = computeEdgePath({ x: sx, y: sy }, { x: tx, y: ty });

    // Style
    ctx.strokeStyle = selectedIds.has(edge.id) ? "#0ea5e9" : (edge.color ?? "#64748b");
    ctx.lineWidth = 2;
    ctx.setLineDash(edge.dashed ? [8, 6] : []);
    drawCubic(ctx, { x: sx, y: sy }, { x: tx, y: ty }, { x: cx1, y: cy1 }, { x: cx2, y: cy2 });
