...
export const renderScene = (input: RenderInput) => {
  ...
  if (input.connecting) {
    ...
    if (fromNode) {
      ...
      const sx = (fromNode.position.x + fromNode.size.width/2) * zoom + offset.x;
      const sy = (fromNode.position.y + fromNode.size.height/2) * zoom + offset.y;
  ...
  drawNodes(ctx, input.nodes, input.viewport, selectedNodeSet, input.hoveredNodeId);
