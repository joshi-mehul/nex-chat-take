...
export const renderScene = (input: RenderInput, backgroundColor?: string) => {
  const ctx = setupCanvas(input.canvas);
  clearAndGrid(ctx, input.viewport, backgroundColor);
  ...
  // Marquee on top
  if (input.marquee) {
    drawMarquee(ctx, input.marquee.start, input.marquee.end);
  }
};
